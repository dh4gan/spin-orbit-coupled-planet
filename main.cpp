...
int readParameters(ifstream &inputStream, string &prefixString,
	bool &fullOutput, double &dt, double &timeMultiplier, int &nLatitude,
	int &nLongitude, double &PsToPo, double &Pspin, double &mstar,
	double &radstar, double &lstar, double &Teff, double &mplanet,
	double &albedo, double &obliquity, double &semi_maj, double &ecc,
...
int main()
    ...
    success = readParameters(myfile, prefixString, fullOutput, dt,
	    timeMultiplier, nLatitude, nLongitude, PsToPo, Pspin, mstar,
	    radstar, lstar, Teff, mplanet, albedo, obliquity, semimaj, ecc, inc,
	    longascend, argper);
    if (success == -1)
	{
	...
	lstar = 4.0 * pi * radstarSI * radstarSI * stefan * Teff * Teff * Teff
		* Teff;
	lstar = lstar / lsol;
	}
    ...
    fprintf(info, "%i \n", nTime);
    fprintf(info, "%+.4E %+.4E %+.4E \n", star.getRadius(), star.getTeff(),
	    star.calculatePeakWavelength());
    fflush(info);
...
#pragma omp parallel default(none) \
	shared(star,planet,j,longitude,latitude,hourAngle,flux,nLatitude)\
	shared(altitude,azimuth,time,Pspin,obliquity,Ephot,micromol) \
	shared(fluxsol,darkness,integratedflux,dt,planetTeff,Ngamma) \
	shared(albedo,stefan)\
		private(k) \
...
#pragma omp for schedule(runtime) ordered
		for (int k = 0; k < nLatitude; k++)
		    ...
		    calcFlux(star, planet, longitude[j], latitude[k],
			    hourAngle[j], flux[j][k], altitude[j][k],
			    azimuth[j][k], time, Pspin, obliquity);
		    ...
		    planetTeff[j][k] = pow(flux[j][k] * (1.0 - albedo) / stefan,
			    0.25);
	...
	fprintf(outputlog,
		...
		time, Porbit, Pspin, (planet.getPosition()).magVector(),
		planet.getTrueAnomaly(), planet.getPosition().elements[0],
	...
	if (fullOutput)
		...
		for (int k = 0; k < nLatitude; k++)
		    ...
		    fprintf(output,
			    ...
			    longitude[j], latitude[k], flux[j][k],
			    planetTeff[j][k], Ngamma[j][k], darkness[j][k],
...
int readParameters(ifstream &inputStream, string &prefixString,
	...
	int &nLongitude, double &PsToPo, double &Pspin, double &mstar,
	double &radstar, double &lstar, double &Teff, double &mplanet,
	double &albedo, double &obliquity, double &semimaj, double &ecc,
	double &inc, double &longascend, double &argper)
    {

    string par, line, fullString;
    double mplanetearth;
...
void calcFlux(Star &star, Planet &planet, double &longitude, double &latitude,
    ...
    Vector3D surface(sin(latitude) * cos(long_apparent),
	    sin(latitude) * sin(long_apparent), cos(latitude));
    ...
    altitude = -cos(declination) * cos(hourAngle) * sin(latitude)
	    + sin(declination) * cos(latitude); // These calculations assume lat->0,180 deg
	...
	azimuth = (sin(altitude) * sin(latitude) - sin(declination))
		/ (cos(altitude) * sin(latitude)); // These calculations assume lat->0, 180 deg
